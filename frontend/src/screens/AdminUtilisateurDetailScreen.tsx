...
import { 
  recupererUtilisateurAdmin, 
  creerLitige, 
  leverSanction, 
  demarrerConversationAdmin,
  basculerActivationCompte, 
  supprimerPublicationAdmin 
} from '../services/admin';
import { recupererPublications } from '../services/publications';
import { Publication, Utilisateur } from '../types';
...
export default function AdminUtilisateurDetailScreen({ route, navigation }: any) {
  ...
  const [utilisateur, setUtilisateur] = useState<Utilisateur | any>(null);
  ...
  async function charger() {
    ...
    try {
      ...
      if (u?.profil_prestataire) {
        try {
          const pubs = await recupererPublications();
          setPublications(pubs);
        } catch {
          // Si les publications ne chargent pas, on ne bloque pas l'écran
        }
      }
    } catch {
      Alert.alert('Erreur', 'Impossible de charger les informations de cet utilisateur.');
    } finally {
      setChargement(false);
    }
  }
  ...
  const publicationsDeCetUtilisateur = publications.filter((p: any) => {
    if (!utilisateur?.profil_prestataire) return false;
    const prestId = typeof p.prestataire === 'object' ? p.prestataire?.id : p.prestataire;
    return prestId === utilisateur.profil_prestataire.id;
  });
  ...
  async function handleSignaler() {
    ...
    try {
      // ✅ Correction de la signature : Transmission d'un objet { utilisateurCibleId, motif }
      await creerLitige({ utilisateurCibleId: utilisateurId, motif: motifLitige });
      Alert.alert('Signalement enregistré', 'Le litige a été créé avec succès.');
      setMotifLitige('');
  ...
  async function handleMessage() {
    try {
      ...
      navigation.navigate('AdminConversation', { 
        ...
        nomInterlocuteur: utilisateur?.username || utilisateur?.first_name 
      });
    } catch {
    ...
    <FlatList
      ...
      ListHeaderComponent={
        <>
          <View style={styles.entete}>
            ...
            <Text style={styles.titre}>{utilisateur.username || utilisateur.email}</Text>
            <View style={{ width: 24 }} />
          </View>
          ...
          <View style={styles.carteInfos}>
            ...
            <Text style={styles.ligne}>Téléphone : {utilisateur.telephone || 'Non renseigné'}</Text>
            <Text style={styles.ligne}>Rôle : {utilisateur.role || 'Client'}</Text>
            <Text style={styles.ligne}>Compte actif : {utilisateur.is_active ? 'Oui' : 'Non'}</Text>

            {utilisateur.est_bloque && (
              <Text style={[styles.ligne, { color: '#DA1E28', fontWeight: '600' }]}>
                BLOQUÉ — {utilisateur.motif_sanction || 'Aucun motif'}
              </Text>
            )}

            {utilisateur.est_suspendu && !utilisateur.est_bloque && (
              <Text style={[styles.ligne, { color: '#F1C21B', fontWeight: '600' }]}>
                SUSPENDU jusqu'au {utilisateur.date_fin_suspension ? new Date(utilisateur.date_fin_suspension).toLocaleDateString('fr-FR') : 'N/A'} — {utilisateur.motif_sanction || 'Aucun motif'}
              </Text>
            )}
            ...
            {utilisateur.profil_client && (
              <Text style={styles.ligne}>
                Adresse : {utilisateur.profil_client.adresse_habituelle || 'Non renseignée'}
              </Text>
            )}
          </View>
          ...
          <View style={styles.rangeeActions}>
            ...
            <TouchableOpacity style={stylesPartages.boutonContour} onPress={handleBasculerActivation}>
              <Text style={stylesPartages.boutonContourTexte}>
      ...
      renderItem={({ item }: { item: any }) => (
        <View style={styles.cartePublication}>
          <Image source={{ uri: item.image || item.photo || IMAGE_PLACEHOLDER }} style={styles.imagePublication} />
          <View style={{ flex: 1 }}>
            <Text numberOfLines={2} style={styles.texteCartePublication}>
              {item.contenu || item.description || item.titre}
            </Text>
          </View>
